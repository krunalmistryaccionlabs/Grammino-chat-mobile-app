...
function autoMessage(props) {
  ...
  const renderChatMessages = () => {
    return props.storageData.helpMessages.map(item => {
      return item.sender === props.storageData.storageData.phone ? (
              ...
              >
                ...
                >
                  <FastImage
                    ...
                    source={{
                      uri: item.ipfsPath
                        ? imageEnvironment + item.ipfsPath
                        : img,
          ...
          {item.type === "video" ? (
            ...
            >
              <TouchableOpacity
                ...
                onPress={() =>
                  Actions.PlayVideo({
                    videoUrl: imageEnvironment + item.ipfsPath,
                    userName: "User"
                  })
                }
              >
  ...
  const selectVideoTapped = () => {
    ...
    ImagePicker.showImagePicker(options, response => {
      if (response.didCancel) {
        ...
        postChatImageHelp(props.storageData.userToken, formData)
          .then(result => {
    ...
    return (
      ...
      >
        ...
        >
          ...
          >
            <IconFour name="md-attach" color="gray" size={width * 0.06} />
          </TouchableOpacity>
        </View>
        ...
        >
          <Icon name="send" color="white" size={width * 0.06} />
        </TouchableOpacity>
      </View>
    );
  };
  ...
  const sendMessage = () => {
    if (message !== "") {
      ...
      setMessage("");
      postHelpMessage(dataHelp, props.storageData.userToken)
        .then(result => {})
        .catch(err => {
          console.log(err);
        });
