...
function contactList(props) {
    ...
    const syncContacts = async () => {
        ...
        PermissionsAndroid.request(
            ...
            Contacts.getAll((err, contacts) => {
                contacts.map((item) => {
                    let temp = item.phoneNumbers[0].number.replace(/[^0-9]/g, "");
                    if (temp !== userPhone) {
                        data.push(temp)
                    }
                })

                updateContacts(data, props.token).then(result => {
        ...
        return (
            <CardView
                ...
                <View
                    ...
                    <Text style={styles.titleFont}>{props.storageData.lan.contacts_value}</Text>
    ...
    function onPressContact(item) {
        ...
        startConversation(item.phone, props.token).then(result => {
            if (!result.data.error) {
